...
- **N × M grid simulation** — supports non-rectangular levels (see `level_04.txt`), fully decoupled from rendering
...
Rows may have ragged lengths (see `level_04.txt`'s cross shape) — the grid is padded with floor cells to the widest row.
